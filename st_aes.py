...
with col1:
    ...
    if st.button("Encrypt"):
        ...
        # Save the encrypted text as bytes when the download button is clicked
        bytes_io = io.BytesIO(contents.encode())
        st.write("Text has been encrypted, click download button to save")
        tai = st.download_button(label='Download to file', data=bytes_io.getvalue(), file_name='encrypted_text.txt')
